...
flags.DEFINE_bool(
    'lth_use_random_label',
    default=False,
  help=('Use random labels rather than the true label'
))
...
class ImageNetTFExampleInput(object):
  ...
  def dataset_parser(self, value):
    ...
    if FLAGS.lth_use_random_label:
      label = tf.cast(tf.strings.to_hash_bucket_fast(parsed['image/filename'], 1000), dtype=tf.int32)
    else:
