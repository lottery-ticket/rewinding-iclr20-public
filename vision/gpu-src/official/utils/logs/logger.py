...
class BenchmarkFileLogger(BaseBenchmarkLogger):
  ...
  def __init__(self, logging_dir):
    ...
    metric_fname = os.path.join(self._logging_dir, METRIC_LOG_FILE_NAME)
    if tf.gfile.Exists(metric_fname):
      mode = 'a'
    else:
      mode = 'w'
    self._metric_file_handler = tf.gfile.GFile(metric_fname, mode)
