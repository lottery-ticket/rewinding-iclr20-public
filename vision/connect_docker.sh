...
if [[ -z "${CONTAINER}" ]]; then
    ...
    nvidia-docker run --rm --name "${NAME}" -d -v `pwd`:/home/lth/lth "${NAME}:latest" tail -f /dev/null
    CONTAINER="$(container_id)"
    sudo docker exec "${CONTAINER}" bash -l /home/lth/lth/docker/posthoc_setup.sh
fi


docker exec -it "${CONTAINER}" bash -l /home/lth/lth/misc/tmux_attach.sh
