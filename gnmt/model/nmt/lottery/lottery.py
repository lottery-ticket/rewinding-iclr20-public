...
class CheckpointerHook(tf.train.SessionRunHook, ABC):
    ...
    def _checkpoint(self, iteration, session):
        ...
        finalized = session.graph.finalized
        if finalized:
...
class PruningHook(tf.train.SessionRunHook, ABC):
    def __init__(self, results_dir, pruning_checkpoint, resetting_checkpoint, pruning_method, model_dir, reset_global_step_to):
        super().__init__()
    ...
    def after_create_session(self, session, coordinator):
        ...
        saveable_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
        ...
        with tf.variable_scope('', reuse=True):
            current_ckpt_mask_values = session.run([v for v in saveable_vars if is_mask_name(v.op.name)])
        ...
        with tf.variable_scope('', reuse=True):
            masks = [v for v in saveable_vars if is_mask_name(v.op.name)]
